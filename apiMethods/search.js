...
var getTitleIds = (string) => {
  return new Promise((resolve, reject) => {
    resolve(
      axios.get(
        `https://api.watchmode.com/v1/search/?apiKey=${key}&search_field=name&search_value=${string}`
      )
    );
  }).then((res) => {
...
var getTitleDetails = (ids) => {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        resolve(
          axios.get(
            `https://api.watchmode.com/v1/title/${id}/details/?apiKey=${key}&append_to_response=sources`
          )
        );
      })
    );
  ...
  return Promise.all(promises)
    .then((data) => data.map((obj) => obj.data))
    .catch((err) => {
